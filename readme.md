...
## 📊 왜 MSI Afterburner를 사용하는가?

본 프로젝트에서 MSI Afterburner의 센서 데이터를 활용하는 이유는 다음과 같습니다.

1. **무료 공유 메모리 제공**: 별도의 복잡한 드라이버 개발 없이도 실시간 시스템 데이터를 공유 메모리 영역을 통해 안정적으로 제공합니다.
2. **높은 접근성 및 호환성**: 공유 메모리 구조 파싱이 비교적 직관적이며, 기간 제한이나 유료 라이선스 걱정 없이 개인 프로젝트에서 자유롭게 연동이 가능합니다.
3. **목적 부합성**: 본 앱의 모니터링은 제어 로직을 위한 보조적인 수단입니다. 따라서 측정값의 절대적인 정밀도가 기관의 인증을 요구하거나 검증을 받을 필요가 없으며, 현재의 연동 방식만으로도 전력 제어라는 목적을 달성하는 데 충분히 신뢰할 만한 데이터를 제공합니다.
...
* **🖥️ 스마트 듀얼 모니터 자동 절전**
  ...
  * 전체화면 종료 시 **DDC/CI 신호**를 통해 보조 모니터를 즉시 복원하며 기존 창 위치까지 원상복구합니다.
  * 모니터가 DDC/CI 1번 명령을 수신할 수 있는 환경이라면 꺼진 상태에서도 자동으로 켜질 수 있으며, 켜진 뒤 원래의 모니터 배치로 완벽하게 복원됩니다
  * 캡처 도구(`Win+Shift+S`, `PrtScn`) 실행 시 오동작하지 않도록 5초 유예 타이머 및 프로세스 예외 처리가 적용되어 있습니다.
...
* **📊 MSI Afterburner 센서 정밀 연동 및 범용 모듈**
  ...
  * 애프터버너 센서 값을 가져오는 모듈은 **범용적으로 설계**되어 있어, 다른 애플리케이션에서도 함수처럼 간단하게 가져다 쓸 수 있습니다.
  * **사용법 예시:**
    ```rust
